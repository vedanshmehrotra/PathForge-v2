...
from pathforge.db.db import init_db, get_connection
...
def test_multi_submission_profile_sequence():
    """Test that update_topic_profile correctly tracks submission history."""
    suffix = uuid.uuid4().hex[:8]
    username = f"elo_test_{suffix}"
    connection = get_connection()
    try:
        cursor = connection.execute(
            """
            INSERT INTO users (username, email, password_hash, created_at, updated_at)
            VALUES (%s, %s, 'hash', '2026-06-03T00:00:00+00:00', '2026-06-03T00:00:00+00:00')
            RETURNING id
            """,
        ...
        row = connection.execute(
            "SELECT * FROM topic_profiles WHERE user_id = %s AND topic = 'Array'",
            (user_id,)
        ).fetchone()
...
def test_get_weakest_topics_ranks_low_skill_first():
    """Test that get_weakest_topics correctly ranks topics by weakness score."""
    suffix = uuid.uuid4().hex[:8]
    username = f"weak_test_{suffix}"
    connection = get_connection()
    try:
        cursor = connection.execute(
            """
        ...
        connection.execute(
            ...
            VALUES
                (90001, 'Dummy', 'Easy', 'Array', '["hash_map_lookup"]', '[]', 50.0, '2026-06-03T00:00:00+00:00')
        ...
        connection.execute(
            ...
            VALUES
                (%s, 'hash_map_lookup', 700, 5, 1, 1, 0.2, 3, '2026-06-03T00:00:00+00:00', '2026-06-03T00:00:00+00:00'),
                (%s, 'bfs_level_order', 1100, 5, 4, 4, 0.8, 0, '2026-06-03T00:00:00+00:00', '2026-06-03T00:00:00+00:00')
            """,
            (user_id, user_id),
        )
        connection.execute(
            ...
            INSERT INTO problems (id, title, difficulty, topics, pattern, test_cases, acceptance_rate, created_at)
            VALUES
                (90002, 'Dummy2', 'Easy', 'Graph', '["bfs_level_order"]', '[]', 50.0, '2026-06-03T00:00:00+00:00')
            ON CONFLICT (id) DO NOTHING
            """
        )
        connection.commit()

        weakest = get_weakest_topics(connection, user_id=user_id)
        assert weakest[0]["topic"] == "hash_map_lookup"
        assert weakest[0]["weakness_score"] > weakest[1]["weakness_score"]
    finally:
        connection.execute("DELETE FROM topic_profiles WHERE user_id = %s", (user_id,))
        connection.execute("DELETE FROM users WHERE id = %s", (user_id,))
        connection.execute("DELETE FROM problems WHERE id IN (90001, 90002)")
        connection.commit()
        connection.close()
